...
const inputjson = "ovon_verbose.json"

function main(){
	var input = JSON.parse(fs.readFileSync(inputjson, 'utf8'));

	var events = input.ovon.events.map((event) => {
		if (event.parameters["dialog-event"]) {
			console.log(event.parameters["dialog-event"].span);
			var output = {
			    ...
			    "participants": {
			        "from": {
			            ...
			            "avatar_url": input.ovon.conversation.speakers[0]["avatar-url"],
			            "displayname": input.ovon.conversation.speakers[0]["display-name"],
			        },
			        "to": {
